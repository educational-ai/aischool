...
def fig_prox_vs_sub():
    ...
    a1.plot(it, zeros_prox, color=GREEN, lw=2.4, label="шаг с порогом")
    a1.plot(it, zeros_sub, color=RED, lw=2.4, label="обычный шаг")
    a1.set_xscale("log")
    ...
    a2.semilogy(xs, np.maximum(np.abs(w_sub[idx]), 1e-6), color=RED, lw=0, marker="o",
                ms=6, label="обычный шаг: нулей нет")
    vis = np.maximum(np.abs(w_prox[idx]), 1e-6)
    a2.semilogy(xs, vis, color=GREEN, lw=0, marker="s", ms=6, label="шаг с порогом: три нуля")
    ...
    a2.set_title(f"Наименьший вес обычного шага: {min_abs_sub:.5f}".replace(".", ","), fontsize=12.5)
    a2.grid(True, color=GRID, lw=0.6, alpha=0.6, axis="y"); a2.set_axisbelow(True)
    a2.legend(frameon=False, fontsize=10, loc="upper left")

    fig.suptitle("Точные нули делает последний шаг, а не сам штраф", y=1.03, fontsize=13.5)
    fig.tight_layout()
