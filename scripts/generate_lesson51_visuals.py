...
bike-sharing data halves the least-squares slope while Huber holds; a high-leverage point
...
# --- normalization guards: these fail if load_diabetes(scaled=True) sneaks back in
assert abs(CORR_S1S2 - 0.897) < 0.002, CORR_S1S2
assert abs(OLS_W[I_S1] + 37.7) < 1.0, OLS_W[I_S1]
assert abs(OLS_W[I_S2] - 22.7) < 1.0, OLS_W[I_S2]
assert abs(RIDGE10[I_S1] + 11.3) < 0.5 and abs(RIDGE10[I_S2] - 1.8) < 0.5
assert abs(RIDGE100[I_S1] + 2.1) < 0.5 and abs(RIDGE100[I_S2] + 3.7) < 0.5
assert abs(OLS_W[NAMES.index("bmi")] - 24.7) < 0.5
...
print(f"5-fold CV R2: OLS {CV_OLS:.3f}  ridge100 {CV_RIDGE100:.3f}  lasso1 {CV_LASSO1:.3f}")
assert abs(CV_OLS - 0.489) < 0.002 and abs(CV_RIDGE100 - 0.482) < 0.002
assert abs(CV_LASSO1 - 0.490) < 0.002

EIG = np.linalg.eigvalsh(Z.T @ Z)
KAPPA0 = EIG.max() / EIG.min()
KAPPA_A = {a: (EIG.max() + a) / (EIG.min() + a) for a in (10, 100, 4420, 44200)}
print(f"eig min={EIG.min():.2f} max={EIG.max():.1f} kappa={KAPPA0:.0f}")
print("kappa(alpha): " + "  ".join(f"{a}:{v:.2f}" for a, v in KAPPA_A.items()))
assert abs(KAPPA0 - 470) < 2
assert abs(KAPPA_A[10] - 129.8) < 1 and abs(KAPPA_A[100] - 18.1) < 0.5
assert abs(KAPPA_A[4420] - 1.40) < 0.02 and abs(KAPPA_A[44200] - 1.04) < 0.02

PATH_ALPHAS = [45.2, 16.8, 6.3, 2.3, 0.87, 0.33, 0.12, 0.045]
PATH_W = np.array([Lasso(alpha=a, max_iter=200000).fit(Z, Y).coef_ for a in PATH_ALPHAS])
PATH_NNZ = [int(np.sum(np.abs(w) > 1e-8)) for w in PATH_W]
print("lasso path nnz:", PATH_NNZ)
assert PATH_NNZ == [0, 3, 4, 7, 8, 8, 10, 10], PATH_NNZ
first3 = {NAMES[i] for i in np.nonzero(np.abs(PATH_W[1]) > 1e-8)[0]}
assert first3 == {"bmi", "bp", "s5"}, first3
...
assert abs(ENTRY["s1"] - 3.27) < 0.2 and abs(ENTRY["s2"] - 0.256) < 0.02

LASSO1 = Lasso(alpha=1.0, max_iter=200000).fit(Z, Y).coef_
ENET = ElasticNet(alpha=1.0, l1_ratio=0.5, max_iter=200000).fit(Z, Y).coef_
print(f"lasso1  s1={LASSO1[I_S1]:.2f} s2={LASSO1[I_S2]:.2f} nnz={int(np.sum(np.abs(LASSO1) > 1e-8))}")
print(f"enet    s1={ENET[I_S1]:.2f} s2={ENET[I_S2]:.2f} nnz={int(np.sum(np.abs(ENET) > 1e-8))}")
assert abs(LASSO1[I_S2]) < 1e-8 and abs(LASSO1[I_S1] + 4.84) < 0.1
assert abs(ENET[I_S1] + 0.24) < 0.05 and abs(ENET[I_S2] + 2.37) < 0.05
assert int(np.sum(np.abs(ENET) > 1e-8)) == 10

# ------------------------------------------------------------------ bike data
def bike_evening(n=60, seed=51):
    t, c = [], []
    with open(BIKE) as f:
...
print(f"bike OLS   {OLS_CLEAN[0]:.1f} -> {OLS_SPIKE[0]:.1f}")
print(f"bike Huber {HUB_CLEAN[0]:.1f} -> {HUB_SPIKE[0]:.1f}   spike at T={BX[SPIKE_J]:.1f}")
assert abs(OLS_CLEAN[0] - 13.7) < 0.15 and abs(OLS_SPIKE[0] - 6.97) < 0.15
assert abs(HUB_CLEAN[0] - 14.8) < 0.2 and abs(HUB_SPIKE[0] - 13.5) < 0.2
assert abs(BX[SPIKE_J] - 2.3) < 0.1

LEV_X, LEV_Y = 45.0, 1400.0
LX = np.append(BX, LEV_X); LY = np.append(BY, LEV_Y)
OLS_LEV = np.polyfit(LX, LY, 1)
HUB_LEV = huber_fit(LX, LY)
print(f"leverage OLS {OLS_CLEAN[0]:.1f} -> {OLS_LEV[0]:.1f}, Huber {HUB_CLEAN[0]:.1f} -> {HUB_LEV[0]:.1f}")
assert abs(OLS_LEV[0] - 16.4) < 0.2 and abs(HUB_LEV[0] - 16.3) < 0.3

RESID = BY - np.polyval(OLS_CLEAN, BX)
MAD = float(np.median(np.abs(RESID - np.median(RESID))))
SIGMA_HAT = 1.4826 * MAD
print(f"MAD={MAD:.0f}  sigma_hat=1.4826*MAD={SIGMA_HAT:.0f}  delta=1.35*sigma={1.35 * SIGMA_HAT:.0f}")
assert abs(MAD - 130) < 3 and abs(SIGMA_HAT - 193) < 4
...
def fig_bike_robust():
    ...
    ax.scatter(BX, BY, s=42, color=INK, alpha=0.75, zorder=4, label="вечер 17:00, реальные поездки")
    ...
    ax.plot(xs, np.polyval(OLS_CLEAN, xs), color=MUTED, lw=1.8, ls=(0, (5, 3)),
            label=f"МНК без выброса: {OLS_CLEAN[0]:.1f} поездки/°C".replace(".", ","), zorder=7)
    ax.plot(xs, np.polyval(OLS_SPIKE, xs), color=RED, lw=2.6,
            label=f"МНК с выбросом: {OLS_SPIKE[0]:.1f}".replace(".", ","))
    ax.plot(xs, HUB_SPIKE[1] + HUB_SPIKE[0] * xs, color=GREEN, lw=2.6,
            label=f"Хьюбер с выбросом: {HUB_SPIKE[0]:.1f}".replace(".", ","))
    ...
    ax.set_title("Одна строка вдвое уменьшила наклон МНК, Хьюбер устоял")
...
def fig_leverage():
    ...
    xall = LX
    h = 1 / len(xall) + (xall - xall.mean()) ** 2 / np.sum((xall - xall.mean()) ** 2)
    res = LY - np.polyval(OLS_LEV, LX)
    assert h[-1] > 5 * np.median(h), (h[-1], np.median(h))
    print(f"leverage h={h[-1]:.3f} (median {np.median(h):.3f}), residual={res[-1]:.0f}")
    ...
    a1.set_title(f"Рычаг в {h[-1] / np.median(h):.0f} раз выше типичного", fontsize=13)
    a1.annotate(f"h = {h[-1]:.2f} при медиане {np.median(h):.3f}".replace(".", ","),
                xy=(h[-1], abs(res[-1])), xytext=(h[-1] * 0.42, abs(res[-1]) + 60),
...
def fig_delta_mad():
    ...
    print(f"share treated linearly at delta=1.35*sigma: {at135:.2f}")
    assert 0.05 < at135 < 0.30
...
def fig_elastic():
    ...
    ax.bar(x - 0.22, LASSO1, width=0.42, color=RED, alpha=0.85, label=r"lasso, $\alpha=1$")
    ax.bar(x + 0.22, ENET, width=0.42, color=GREEN, alpha=0.85,
           label=r"elastic net, $\alpha=1$, $\ell_1$-доля $0{,}5$")
    ...
    ax.annotate(f"lasso: s2 = 0\nelastic net: {ENET[I_S2]:.2f}".replace(".", ","),
                xy=(I_S2 + 0.22, ENET[I_S2]), xytext=(I_S2 + 1.5, -6.5),
                fontsize=10, color=GREEN, arrowprops=dict(arrowstyle="->", color=GREEN, lw=1.1))
    ax.set_title("Ромб выбирает одного из пары, смесь оставляет обоих")
...
def fig_bias_variance():
    ...
    print(f"bias-variance: best alpha={als[k]:.0f}, MSE {mse[0]:.0f} -> {mse[k]:.0f} -> {mse[-1]:.0f}, "
          f"variance {var[0]:.0f} -> {var[k]:.0f}")
    assert 30 < als[k] < 120
    assert mse[k] < mse[0] * 0.92 and mse[k] < mse[-1] * 0.75
    assert var[0] > 3.5 * var[k]
    ...
    ax.set_title("Обучение по 60 больным: разброс падает быстрее, чем растёт смещение")
    ax.legend(frameon=False, fontsize=10, loc="upper left")
    ax.grid(True, color=GRID, lw=0.4, alpha=0.5, which="both"); ax.set_axisbelow(True)
